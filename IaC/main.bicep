...
@description('Location for all resources, e.g., westeurope or uksouth')
param location string = resourceGroup().location
...
@description('AI Services (Foundry) account name (globally unique subdomain)')
param aiName string

@description('Default Project name to create under AI Services')
param projectName string = 'documind-dev'

@description('Azure OpenAI account name (custom subdomain)')
param aoaiName string
...
@description('Enable Defender for AI on AI Services')
@allowed(['Enabled', 'Disabled'])
param defenderForAI string = 'Disabled'
...
// ---------------------------------------
// Azure AI Services (Foundry-aligned)
// ---------------------------------------
resource ai 'Microsoft.CognitiveServices/accounts@2025-06-01' = {
  name: aiName
  location: location
...
output searchResourceId string = search.id
output aiEndpoint string = 'https://${ai.name}.services.ai.azure.com/'
output aiResourceId string = ai.id
output projectId string = project.id
