# services/rag_api/routers/ingest_ocr.py
...
router = APIRouter(prefix="/ingest", tags=["ingest-ocr"])
...
@router.post("/upload")
async def ingest_upload(file: UploadFile = File(...), doc_id: str | None = Form(None)):
    raw = await file.read()
...
def _guess_content_type(url: str) -> str:
    ctype, _ = mimetypes.guess_type(url); return ctype or "application/octet-stream"
...
def _ocr_pdf(data: bytes) -> str:
    if fitz is None:
        first = _pdf_first_page_as_png(data); return _ocr_image(first) if first else ""
    try:
...
def _pdf_first_page_as_png(data: bytes) -> bytes:
    if fitz is None: return b""
    try:
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=True) as f:
            f.write(data); f.flush()
            doc = fitz.open(f.name); p = doc[0]
            pix = p.get_pixmap(dpi=200)
            img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)
            bio = BytesIO(); img.save(bio, format="PNG"); bio.seek(0)
            return bio.read()
